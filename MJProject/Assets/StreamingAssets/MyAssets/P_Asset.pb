
ºW
P_Asset.protoAdoter.Asset"Ä
AssetCommonProp
	global_id (+
version (2.Adoter.Asset.VERSION_TYPE
name (
	show_name (
desc ("í
RoomOptions,
model (2.Adoter.Asset.ROOM_MODEL_TYPE
extend_type_count (:103
extend_type (2.Adoter.Asset.ROOM_EXTEND_TYPE
top_mutiple (

open_rands (:8

voice_open (

gmt_opened (-
pay_type (2.Adoter.Asset.ROOM_PAY_TYPE"Ÿ
	RoomLimit9
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_ROOM2
common_prop (2.Adoter.Asset.AssetCommonProp*
	room_type (2.Adoter.Asset.ROOM_TYPE
	min_limit (
	max_limit (

cost_count (

base_count (/
room_options (2.Adoter.Asset.RoomOptions
reconnect_timeout	 (
reconnect_count
 ("—
CommonLimitA
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_COMMON_LIMIT2
common_prop (2.Adoter.Asset.AssetCommonProp?

limit_type (2+.Adoter.Asset.CommonLimit.COMMON_LIMIT_TYPE@
cool_down_type (2(.Adoter.Asset.CommonLimit.COOL_DOWN_TYPEL
cool_down_clear_type (2..Adoter.Asset.CommonLimit.COOL_DOWN_CLEAR_TYPE
	max_count ("v
COMMON_LIMIT_TYPE
COMMON_LIMIT_TYPE_PLANT!
COMMON_LIMIT_TYPE_LUCKY_PLATE!
COMMON_LIMIT_TYPE_DIALY_BONUS"é
COOL_DOWN_TYPE
COOL_DOWN_TYPE_DAY
COOL_DOWN_TYPE_WEEK
COOL_DOWN_TYPE_MONTH
COOL_DOWN_TYPE_YEAR
COOL_DOWN_TYPE_NEVER"^
COOL_DOWN_CLEAR_TYPE"
COOL_DOWN_CLEAR_TYPE_ZERO_TIME"
COOL_DOWN_CLEAR_TYPE_WHEN_OPER"¡
CoolDown>
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_COOL_DOWN2
common_prop (2.Adoter.Asset.AssetCommonProp
duration ("/
COOL_DOWN_REASON
COOL_DOWN_REASON_NORMAL"ø
MJCard<
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_MJ_CARD2
common_prop (2.Adoter.Asset.AssetCommonProp*
	card_type (2.Adoter.Asset.CARD_TYPE
group_count (:4
cards_count (:9(
cards (2.Adoter.Asset.MJCard.Card=
Card
value (

model_path (

voice_path ("C
ActivityPage
activity_id (
	page_name (
url ("⁄
CommonConstA
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_COMMON_CONST2
common_prop (2.Adoter.Asset.AssetCommonProp
model_release (
max_cards_count (:136
player_count (:4
daily_bonus_id (
daily_allowance_id (
operation_time_out (:8
daily_sign_id	 (
fan_id
 (
room_card_id (
liuju_count (
room_last_time ((
 room_card_limit_free_activity_id ( 
guest_forbid_friend_room (
mall_activity_id (!
room_dismiss_timeout (:120"
room_dismiss_cooldown (:180
room_aapay_diamond (:3
limit_version (
page_list_count (:5-
	page_list (2.Adoter.Asset.ActivityPage
room_card_beishu (:4"‘
CommonRewardB
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_COMMON_REWARD2
common_prop (2.Adoter.Asset.AssetCommonProp
rewards_count (:102
rewards (2!.Adoter.Asset.CommonReward.Reward
common_limit_id (ä
Reward;
reward_type (2&.Adoter.Asset.CommonReward.REWARD_TYPE
comments (
	icon_name (
item_id (
count ("W
REWARD_TYPE
REWARD_TYPE_DIAMOND
REWARD_TYPE_HUANLEDOU
REWARD_TYPE_ITEM"™
CommonConsumeC
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_COMMON_CONSUME2
common_prop (2.Adoter.Asset.AssetCommonProp
elements_count (:35
elements (2#.Adoter.Asset.CommonConsume.Elementq
Element>
consume_type (2(.Adoter.Asset.CommonConsume.CONSUME_TYPE
item_id (
consume_count ("[
CONSUME_TYPE
CONSUME_TYPE_DIAMOND
CONSUME_TYPE_HUANLEDOU
CONSUME_TYPE_ITEM"ˇ

LuckyPlate@
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_LUCKY_PLATE2
common_prop (2.Adoter.Asset.AssetCommonProp
plates_count (:10.
plates (2.Adoter.Asset.LuckyPlate.Plate1
Plate
weight (
common_reward_id ("ú

DailyBonus@
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_DAILY_BONUS2
common_prop (2.Adoter.Asset.AssetCommonProp
common_reward_id ("ç
Mall9
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_MALL2
common_prop (2.Adoter.Asset.AssetCommonProp1

goods_type (2.Adoter.Asset.Mall.GOODS_TYPE
item_id (
count (
diamond (
diamond_show (
	huanledou (
huanledou_show	 (
activity_id
 ("\

GOODS_TYPE
GOODS_TYPE_HUANLEDOU
GOODS_TYPE_NORMAL_ITEM
GOODS_TYPE_ROOM_CARD"ú
RoomFan=
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_ROOM_FAN2
common_prop (2.Adoter.Asset.AssetCommonProp

fans_count (:30.
fans (2 .Adoter.Asset.RoomFan.FanElementV

FanElement(
fan_type (2.Adoter.Asset.FAN_TYPE
name (
multiple ("Ä
	RoomModel?
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_ROOM_MODEL2
common_prop (2.Adoter.Asset.AssetCommonProp"˝
RoomOpen>
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_ROOM_OPEN2
common_prop (2.Adoter.Asset.AssetCommonProp
costs_count (:10.
costs (2.Adoter.Asset.RoomOpen.CardCost4
CardCost
rounds (
room_cards_count ("Ö

VoiceAlter@
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_VOICE_ALTER2
common_prop (2.Adoter.Asset.AssetCommonProp
voices_count (:105
voices (2%.Adoter.Asset.VoiceAlter.VoiceElement0
VoiceElement
name (

voice_path ("ù
Face9
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_FACE2
common_prop (2.Adoter.Asset.AssetCommonProp
faces_count (:20
faces ("•
Avatar;
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_AVATAR2
common_prop (2.Adoter.Asset.AssetCommonProp
avatars_count (:20
avatars ("ˇ
Activity=
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_ACTIVITY2
common_prop (2.Adoter.Asset.AssetCommonProp2
activity_type (2.Adoter.Asset.ACTIVITY_TYPE

start_date (

start_time (
	stop_date (
	stop_time ("√
DailyAllowanceD
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_DAILY_ALLOWANCE2
common_prop (2.Adoter.Asset.AssetCommonProp
huanledou_below (:2000
common_reward_id ("
Recharge=
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_RECHARGE2
common_prop (2.Adoter.Asset.AssetCommonProp
	plat_name (

product_id (
	icon_name (
price (

price_show (
gain_diamond ("◊
	DailySign?
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_DAILY_SIGN2
common_prop (2.Adoter.Asset.AssetCommonProp
common_reward_id (
common_limit_id (
signs_count (:105
signs (2&.Adoter.Asset.DailySign.ContinuousSignR
ContinuousSign
count (
common_reward_id (
common_limit_id ("W
MailAttachment6
attachment_type (2.Adoter.Asset.ATTACHMENT_TYPE
count ("ı

SystemMail9
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_MAIL2
common_prop (2.Adoter.Asset.AssetCommonProp*
	mail_type (2.Adoter.Asset.MAIL_TYPE?
title (:0\347\263\273\347\273\237\351\202\256\344\273\266&
from (:\347\263\273\347\273\237

content_id (
attachments_count (:31
attachments (2.Adoter.Asset.MailAttachment"Ù
SystemSpeakA
type_t (2.Adoter.Asset.ASSET_TYPE:ASSET_TYPE_SYSTEM_SPEAK2
common_prop (2.Adoter.Asset.AssetCommonProp

text_index ( 
infomation_list_count (:28
infomation_list (2.Adoter.Asset.SYSTEM_SPEAK_TYPE"û
Item_CommonProp2
common_prop (2.Adoter.Asset.AssetCommonProp
quality (:1
min_level_limit (
max_level_limit (
pile_max (:1
count (:1
	batch_use (
auto_use (
cool_down_id	 (/
	inventory
 (2.Adoter.Asset.INVENTORY_TYPE"j
	Item_Item9
type_t (2.Adoter.Asset.ASSET_TYPE:ITEM_TYPE_BEGIN
stuff (
stuff_extra ("Ç
Item_Normal:
type_t (2.Adoter.Asset.ASSET_TYPE:ITEM_TYPE_NORMAL7
item_common_prop (2.Adoter.Asset.Item_CommonProp"∏
Item_Potion:
type_t (2.Adoter.Asset.ASSET_TYPE:ITEM_TYPE_POTION7
item_common_prop (2.Adoter.Asset.Item_CommonProp:
potion_type (2%.Adoter.Asset.Item_Potion.POTION_TYPE
increase_value (:1"]
POTION_TYPE
POTION_TYPE_HEMO
POTION_TYPE_MAGIC!
POTION_TYPE_PHYSICAL_STRENGTH"ö
Item_RoomCard=
type_t (2.Adoter.Asset.ASSET_TYPE:ITEM_TYPE_ROOM_CARD7
item_common_prop (2.Adoter.Asset.Item_CommonProp
rounds (:1*Ñ
INVENTORY_TYPE
INVENTORY_TYPE_BACKPACKAGE
INVENTORY_TYPE_EQUIPMENT
INVENTORY_TYPE_FASHION
INVENTORY_TYPE_COUNT*9
GENDER_TYPE
GENDER_TYPE_MAN
GENDER_TYPE_WOMAN*[
VERSION_TYPE
VERSION_TYPE_GLOBAL
VERSION_TYPE_MAINLAND
VERSION_TYPE_TAIWAN*◊

ASSET_TYPE
ASSET_TYPE_BEGIN
ASSET_TYPE_ROOM
ASSET_TYPE_COMMON_LIMIT
ASSET_TYPE_COMMON_REWARD
ASSET_TYPE_COOL_DOWN
ASSET_TYPE_MJ_CARD
ASSET_TYPE_COMMON_CONST
ASSET_TYPE_LUCKY_PLATE
ASSET_TYPE_DAILY_BONUS	
ASSET_TYPE_MALL

ASSET_TYPE_ROOM_FAN
ASSET_TYPE_ROOM_MODEL
ASSET_TYPE_ROOM_OPEN
ASSET_TYPE_VOICE_ALTER
ASSET_TYPE_FACE
ASSET_TYPE_AVATAR
ASSET_TYPE_ACTIVITY
ASSET_TYPE_DAILY_ALLOWANCE
ASSET_TYPE_COMMON_CONSUME
ASSET_TYPE_RECHARGE
ASSET_TYPE_DAILY_SIGN
ASSET_TYPE_MAIL
ASSET_TYPE_SYSTEM_SPEAK
ASSET_TYPE_COUNT»
ITEM_TYPE_BEGIN…
ITEM_TYPE_POTION 
ITEM_TYPE_ROOM_CARDÀ
ITEM_TYPE_NORMALÃ*d
	ROOM_TYPE
ROOM_TYPE_FRIEND
ROOM_TYPE_XINSHOU
ROOM_TYPE_GAOSHOU
ROOM_TYPE_DASHI*É
ROOM_EXTEND_TYPE
ROOM_EXTEND_TYPE_NULL 
ROOM_EXTEND_TYPE_ZHANLIHU
ROOM_EXTEND_TYPE_JIAHU!
ROOM_EXTEND_TYPE_XUANFENGGANG
ROOM_EXTEND_TYPE_BAOPAI
ROOM_EXTEND_TYPE_DUANMEN
ROOM_EXTEND_TYPE_QIYISE
ROOM_EXTEND_TYPE_BAOSANJIA*N
ROOM_MODEL_TYPE
ROOM_MODEL_TYPE_CLASSICAL
ROOM_MODEL_TYPE_MULTIPLE*?
ROOM_PAY_TYPE
ROOM_PAY_TYPE_HOSTER
ROOM_PAY_TYPE_AA*t
	CARD_TYPE
CARD_TYPE_WANZI
CARD_TYPE_BINGZI
CARD_TYPE_TIAOZI
CARD_TYPE_FENG
CARD_TYPE_JIAN*˝
FAN_TYPE
FAN_TYPE_PINGHU 
FAN_TYPE_ZHUANG
FAN_TYPE_DIAN_PAO
FAN_TYPE_ZI_MO
FAN_TYPE_SHANG_TING
FAN_TYPE_BIMEN
FAN_TYPE_HAI_DI_LAO
FAN_TYPE_GANG_SHANG_KAI
FAN_TYPE_LIU_LEI
FAN_TYPE_LOU_BAO	
FAN_TYPE_PIAO_HU

FAN_TYPE_SI_GUI_YI
FAN_TYPE_MING_PIAO
FAN_TYPE_SAN_JIA_BI_MEN
FAN_TYPE_QING_YI_SE
FAN_TYPE_MING_GANG
FAN_TYPE_AN_GANG
FAN_TYPE_JIN_BAO
FAN_TYPE_ZHAN_LI
FAN_TYPE_DUAN_MEN
FAN_TYPE_XUAN_FENG_GANG
FAN_TYPE_JIA_HU_NORMAL
FAN_TYPE_JIA_HU_MIDDLE
FAN_TYPE_JIA_HU_HIGHER
FAN_TYPE_PIAO_WEIHU
FAN_TYPE_BAOSANJIA*P
ACTIVITY_TYPE
ACTIVITY_CYCLE_TYPE_DIALY 
ACTIVITY_CYCLE_TYPE_DURATION*:
	MAIL_TYPE
MAIL_TYPE_HELP
MAIL_TYPE_ANNOUCEMENT*l
ATTACHMENT_TYPE
ATTACHMENT_TYPE_DIAMOND
ATTACHMENT_TYPE_HUANLEDOU
ATTACHMENT_TYPE_ROOM_CARD*W
SYSTEM_SPEAK_TYPE
SYSTEM_SPEAK_TYPE_PLAYER_ID!
SYSTEM_SPEAK_TYPE_PLAYER_NAME